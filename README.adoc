...
$ cd petstore-webmvc
$ metacode create project spring-webmvc --name petstore --package acme.petstore --base-path /petstore --add openapi
----
...
Try these URLs with the Httpie command, or in your browser to see what happens
[source,bash]
----
# Renders the OpenAPI page for the Pet Store
$ http localhost:8080/petstore/swagger-ui/index.html

# Renders the OpenAPI documentation
$ http localhost:8080/petstore/v3/api-docs
...
# Returns an error because the pet ID is not given
$ http localhost:8080/petstore/pets

# Returns the given Pet
$ http localhost:8080/petstore/pets/[resourceId]
